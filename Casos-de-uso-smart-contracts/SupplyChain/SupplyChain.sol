...
contract SupplyChain {
    ...
    function registerProduct(uint256 productId, uint256 _price) public returns (bool success) {
        require(products[productId].length == 0, "This product already exists");
        products[productId].push(Step(Status.CREATED, "", _price, msg.sender));
        return success;
    }
    ...
    function registerStep(uint256 productId, string calldata metadata, uint256 price) public payable returns (bool success) {
        ...
        if (currentStatus == uint256(Status.PICKED_UP) || currentStatus == uint256(Status.DELIVERED)) {
